...
fn signal_protocol(module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    for module_name in mods.iter() {
        ...
        module.py().run(&cmd, None, Some(&module.dict()))?;
    }
    Ok(())
}
